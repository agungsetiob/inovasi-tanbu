                                        ...
                                        @forelse ($indikator->files()->where('proposal_id', $proposalId)->get() as $item)
                                            ...
                                            @if ($item->file)
                                                <a class="btn btn-outline-success btn-sm" title="download"
                                                    href="{{url('/storage/docs/'. $item->file )}}" target="_blank">
                                                    <i class="fa-solid fa-download"></i>
                                                </a>
                                            @endif
                                        @empty
                                            <button class="btn-add btn btn-outline-primary btn-sm"
                                                data-id="{{$indikator->id}}" title="upload" href="#"
                                    ...
                                    @elseif ($proposal->status === 'sent')
                                        @foreach ($indikator->files()->where('proposal_id', $proposalId)->get() as $item)
                                            ...
                                            @elseif (!$item->file && $item->informasi)
                                                <a class="text-danger" title="open video" href="{{$item->informasi}}"
                                                    target="_blank">
